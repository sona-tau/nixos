{ config, pkgs, lib, myLib, ... }: let cfg = config.my.roles; in {
	options = {
		my.roles = {
	...
	config = {
		...
		in with myLib; 
		   (catIf cfg.base.pkgSet.enable base)
		++ (catIf cfg.email.pkgSet.enable email)
		++ (catIf cfg.wayland.pkgSet.enable wayland)
		++ (catIf cfg.terminal.pkgSet.enable terminal)
		++ (catIf cfg.browsers.pkgSet.enable browsers)
		++ (catIf cfg.zen.pkgSet.enable zen)
		++ (catIf cfg.webdev.pkgSet.enable webdev)
		++ (catIf cfg.lean.pkgSet.enable lean)
		++ (catIf cfg.llm.pkgSet.enable llm)
		++ (catIf cfg.fun.pkgSet.enable fun)
		++ (catIf cfg.writing.pkgSet.enable writing);
	};
}
