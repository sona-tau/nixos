{ config, pkgs, lib, ... }: {
	config = {
		home.packages = [ (lib.hiPrio pkgs.swayfx) ];
		...
		wayland.windowManager.sway = {
			...
			config = rec {
				...
				terminal = if config.terminal == "alacritty" then "alacritty"
					else if config.terminal == "foot" then "foot" else null;
				...
				input = {
					"1:1:AT_Translated_Set_2_keyboard".xkb_layout = "mtgap-mod";
					"type:touchpad" = {
						tap = "enabled";
